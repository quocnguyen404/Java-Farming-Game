...
{
    //5x4s
    public static Supplier<Crop> onGetCrop;
    ...
    public Region(int x, int y)
    {
        ...
        rect.x *= GameConstant.TILE_WIDTH*GameFrame.X_ZOOM;
        rect.y = y - (rect.h/(2*GameConstant.TILE_HEIGHT*GameFrame.Y_ZOOM));
        rect.y *= GameConstant.TILE_HEIGHT*GameFrame.Y_ZOOM;
    ...
    public boolean mouseDraggedExit(Rectangle mousRectangle, Rectangle camRectangle, int xZoom, int yZoom) 
    {
        for (RegionCell cell : cells)
        {
            if (cell.onHoverCell(mousRectangle) && cell.isEmpty())
            {
